...
InfoBox.prototype.close = function () {
  ...
  this.setMap(null);
	...
	var myBalloonOptions = {
		...
		,boxStyle: { 
			background: "url('wp-content/plugins/events-manager-extended/tipbox.gif') no-repeat"
			,opacity: 0.75
			,width: "280px"
		}
	...
	if (document.getElementById("dbem_global_map")) {
		...
		$j_dbem_locations.getJSON(document.URL,{ajax: 'true', query:'GlobalMapData', eventful:eventful, scope:scope}, function(data) {
			...
			$j_dbem_locations.each(locations, function(i, item) {
				...
				location_info.innerHTML = "<strong>"+ item.location_name+ "</strong><br/>" + item.location_address + ", "+ item.location_town + "<br/><small><a href='" + events_page+ "&location_id=" + item.location_id + "'>Details<a>";
	...
	for (var i = 0; i < divs.length; i++){                      
		...
		if(divname.indexOf("dbem-location-map_") == 0) { 
			...
			//var content= "<div class=\"dbem-location-balloon\">"+map_text_id+"</div>";
